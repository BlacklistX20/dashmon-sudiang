...
class CoolModel extends Model
{
  ...
  protected $allowedFields    = ['barcode', 'floor', 'room', 'category', 'name', 'vendor', 'brand', 'type', 'compressor', 'flow', 'capacity', 'condition', 'status', 'aging', 'info', 'install', 'maintanance', 'updated_at'];
  ...
  public function getFilteredData($search, $start, $length)
  {
    ...
    if (!empty($search)) {
      $builder->groupStart()
        ...
        ->orLike('brand', $search)
        ->orLike('sn_indor', $search)
        ->groupEnd();
    }

    $builder->select('barcode, floor, room, category, name, vendor, brand, type, compressor, flow, capacity, condition, status, aging, info, install, maintanance')
      ->limit($length, $start);
